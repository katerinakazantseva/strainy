...
def build_paths_graph(edge, flye_consensus, SNP_pos, cl, cons, full_clusters, data, ln, full_paths_roots,
                      full_paths_leafs, cluster_distances):
    '''
...
def add_path_edges(edge, g, cl, data, SNP_pos, ln, full_paths, G, paths_roots, paths_leafs, full_clusters, cons,
                   flye_consensus):
    '''
...
def graph_create_unitigs(edge, graph, flye_consensus, bam_cache, link_clusters,
    ...
    cl = None
    try:
        cl = pd.read_csv("%s/clusters/clusters_%s_%s_%s.csv" % (StRainyArgs().output, edge, I, AF),
    ...
    if cl is not None:
        ...
        if len(clusters) > 1:
            for cluster in clusters:
                ...
                if clStart < start_end_gap and clStop > ln - start_end_gap:
                    if strong_tail(cluster, cl, ln, data)[0] == True and strong_tail(cluster, cl, ln,
                                                                                     data)[1] == True:
                        consensus = flye_consensus.flye_consensus(cluster, edge, cl)
                        add_child_edge(edge, cluster, graph, cl, consensus['start'], consensus['end'], cons,
                                       flye_consensus)
            ...
            othercl = list(
                set(clusters) - set(full_clusters) - set([j for i in full_paths for j in i]) - set(cl_removed))
            if len(othercl) > 0:
...
def graph_link_unitigs(edge, graph, bam_cache, link_clusters, link_clusters_src, link_clusters_sink, remove_clusters):
    ...
    for cur_clust in link_unitigs:
        ...
        for next_seg in set({k for k, v in Counter(neighbours.values()).items() if v >= min_reads_neighbour}):
            ...
            try:
                cl_n = pd.read_csv("%s/clusters/clusters_%s_%s_%s.csv" % (StRainyArgs().output, next_seg, I, AF),
                                   keep_default_na=False)
            except(FileNotFoundError):
                add_link(graph, f"{edge}_{cur_clust}", fr_or, next_seg, to_or, 555)
                continue
...
def transform_main(args):
    ...
    flye_consensus = FlyeConsensus(StRainyArgs().bam, StRainyArgs().fa, args.threads, consensus_dict,
