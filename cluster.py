...
def main(edge):
    ...
    cl['ID']=cluster_membership.keys()
    cl.loc[cl['Cluster']=='NA','Cluster']=0
    #cl.to_csv("output/clusters_%s.csv" % edge)

    nx.draw(G, nodelist=G.nodes(), with_labels = False,width=0.03, node_size=3, font_size=5,node_color = cl['Cluster'].astype(int), cmap = plt.cm.rainbow)
    #print(cl)
...
""" #cliques=sorted(list(nx.find_cliques(G)),key=len, reverse=True)
    ...
    clN=0
    cluster_membership={}
    for cluster in sorted(map(sorted, next_level_communities)):
        clN=clN+1
        for i in cluster:
            cl['Cluster'][i] = clN
            cluster_membership[i]=clN
"""
